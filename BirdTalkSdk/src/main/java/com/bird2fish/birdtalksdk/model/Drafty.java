...
public class Drafty implements Serializable {
    ...
    private static final EntityProc[] ENTITY_PROC = {
            // 这个正则表达式用来匹配以 "http://" 或 "https://" 开头的网址，且支持可选的 "www." 子域名。
            new EntityProc("LN",
            ...
            // 这个正则表达式用来匹配以 "@" 开头的用户名，通常用于社交媒体平台中的用户标记。
            new EntityProc("MN",
            ...
            // 这个正则表达式用来匹配以 "#" 开头的标签（如 Twitter 或 Instagram 中的标签）。
            new EntityProc("HT",
                    Pattern.compile("(?<=^|[\\W_])#([\\p{L}\\p{N}][._\\p{L}\\p{N}]*[\\p{L}\\p{N}])")) {
                @Override
                Map<String, Object> pack(Matcher m) {
    ...
    public static Drafty fromPlainText(String plainText) {
        ...
        // 这句代码的作用是对 plainText 字符串进行 Unicode 规范化
    ...
    private void prepareForEntity(int at, int len) {  // , String tp
        prepareForStyle(1);
